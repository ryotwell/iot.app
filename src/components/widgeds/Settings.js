...
function SettingsWidged() {
    ...
    const [setting, setSetting] = useState({ dht_11_real_time_notification: true })
    ...
    const handleOnSave = () => {
        ...
        axios.post('/api/user/setting', { ...setting }).then(() => {
            setSubmit(false)
            toast.success('Update berhasil, lalukan refresh pada browser untuk menerapkan perubahan pada browser.', {
    ...
    useEffect(() => {
        setSetting({
            dht_11_real_time_notification: user.setting.dht_11_real_time_notification,
        })
    }, [])
    ...
    return (
        ...
        >
            <div className="w-full">
                <div className="mb-4">
                    <h3 className="font-semibold text-lg mb-4">In App Notifications</h3>
